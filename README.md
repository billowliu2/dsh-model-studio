...
| Model capabilities | Per-model context window, output cap, image input (inherit / text only / text + image) and thinking levels (inherit / not a reasoning model / any of the levels, with automatic wire-spelling mapping). Fields the editor does not model — `compat`, `thinkingBudgets`, anything pi-ai adds later — survive unchanged. |
...
| Suite | Checks | Covers |
|---|---|---|
| `node test/smoke.mjs` | 160 | registration and disposal, slot wiring, locale completeness (both languages), panel rendering in both hosts, tab switching, drawer open/close/dock, class ↔ stylesheet contract, end-to-end reference enrichment, preset pre-fill and create |
| `node test/api.mjs` | 180 | provider derivation, path-operation semantics, capability draft round-trips, discovery merging, header validation, JSON diffing, credential write order, effort/preset resolution, the generated catalog snapshot |
